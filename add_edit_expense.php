	...
	<section class="features7 cid-sENIyiRsb8" id="features08-3" style="min-height: 500px;">
		...
		<div class="row justify-content-center">
		    <div class="col-lg-12 mx-auto mbr-form form-col md-pb">
				...
				<?php
				if($update){
				    ?><input type="hidden" name="expense_id" value="<?=$expense_id?>"/><?php
				}
				?>
                                <div class="dragArea form-row">
				    <div class="col-sm-12 form-group">
                                        <input type="number" name="expense[amount]" placeholder="Amount" value="<?php echo $update ? $expenseInfo['amount'] : ""; ?>" class="form-control display-7" required="required" />
                                    </div>
				    ...
				    <div class="col-sm-12 form-group">
						...
						?><option <?php if($update && $expenseInfo['category_id'] == $category['id']){ ?>selected="selected"<?php } ?> value="<?=$category['id']?>"><?=$category['name']?></option><?php
					    }
					    ?>
					</select>
                                    </div>
